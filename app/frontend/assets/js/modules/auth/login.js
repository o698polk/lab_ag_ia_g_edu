...
(function () {
  ...
  async function doLogin() {
    ...
    try {
      if (!username || !password) { showStatus(""); showErr("Completa usuario y contraseña."); return; }
      const res = await api("POST", "/auth/login", { username, password }, 15000, false);
      if (!res.ok) { showStatus(""); showErr(friendly(res.status, res.data)); return; }
      if (!res.data?.access_token) { showStatus(""); showErr("El servidor no devolvió token."); return; }
      ...
      if (!me.ok || !me.data) { showStatus(""); showErr("Login OK pero /auth/me falló (" + me.status + ")."); return; }
      state.user = me.data;
      location.assign("/ui/pages/dashboard/dashboard.html");
    } catch (err) {
      showStatus("");
      showErr("No hay respuesta del API. " + (err?.message || ""));
    } finally {
  ...
  document.getElementById("login-form")?.addEventListener("submit", (ev) => { ev.preventDefault(); doLogin(); });
